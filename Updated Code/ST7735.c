...
uint32_t ST7735_DrawString_mod(uint16_t x, uint16_t y, char *pt, int16_t textColor, uint8_t font){
 uint32_t count = 0;
 uint16_t selColor = ~textColor; 
 if(y>15) return 0;
 while(*pt){
	  //ST7735_DrawCharS(x*6, y*10, *pt, textColor, ST7735_BLACK, 1);
	  ST7735_DrawCharS(x*6, y*10, *pt, textColor, selColor, font);
   pt++;
   x = x+1;
   if(x>20) return count;  // number of characters printed
   count++;
